...
import argparse
import torch
from ppo.src.env import MultipleEnvironments
from ppo.src.model import PPO
from ppo.src.process import eval
...
def train(opt):
    ...
    while True:
        ...
        gae = 0
        R = []
        for value, reward, done in list(zip(values, rewards, dones))[::-1]:     # calc advantage
        ...
        for i in range(opt.num_epochs):
            ...
            for j in range(opt.batch_size):
                ...
                ratio = torch.exp(new_log_policy - old_log_policies[batch_indices]) # ratio
                actor_loss = -torch.mean(torch.min(ratio * advantages[batch_indices],
                                                   torch.clamp(ratio, 1.0 - opt.epsilon, 1.0 + opt.epsilon) *
                                                   advantages[
                                                       batch_indices])) # cliping
                ...
                optimizer.zero_grad()
                total_loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 0.5)     # model clip
                optimizer.step()
